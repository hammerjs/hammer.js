...
function Hammer(element, options)
{
    ...
    this.getDirectionFromAngle = function( angle )
    {
        ...
        var direction;
        Object.each(directions, function(value, key){
            if (value){
                direction = key;
                return false;
            }
    ...
    function countFingers( event )
    {
        ...
        return event.event.touches ? event.event.touches.length : 1;
    }
    ...
    function getXYfromEvent( event )
    {
        ...
        // multitouch, return array with positions
            ...
            var pos = [], src;
            for(var t=0, len=event.event.touches.length; t<len; t++) {
                src = event.event.touches[t];
    ...
    function triggerEvent( eventName, params )
    {
        ...
        // trigger mootools event
        element.fireEvent(eventName, params);
        
        // trigger callback
        evName = "on" + eventName.capitalize();
       
        if(typeOf(self[evName]) == 'function') {
            self[evName].attempt(params, self);//.delay(1, self, sendparams);
        }
    }
    ...
    function handleEvents(event)
    {
		...
		// mootools-ize        
		if (!event.originalEvent && event.event){
        ...
        switch(event.type)
        {
            ...
            case 'touchstart':
                ...
                _first = true;
                _offset = element.getCoordinates();//element.offset();
    ...
    if ('ontouchstart' in window){
        ...
        element.addEvents({
        	"touchstart": handleEvents,
        	"touchmove": handleEvents,
        	"touchend" : handleEvents,
        	"touchcancel": handleEvents
        });
    ...
    // for non-touch
        ...
        document.id(document).addEvent("mouseup", handleEvents);
        element.addEvent("mousedown", handleEvents);
        element.addEvent("mousemove", handleEvents);
    }
}